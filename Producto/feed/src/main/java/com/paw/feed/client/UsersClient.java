...
public class UsersClient {
    ...
    public Optional<NgoProfileResponse> findNgoProfile(UUID ngoId) {
        try {
            ResponseEntity<ApiResponse<NgoProfileResponse>> response = restTemplate.exchange(
                    usersBaseUrl + "/ngos/" + ngoId,
                    HttpMethod.GET,
                    HttpEntity.EMPTY,
                    new ParameterizedTypeReference<>() {}
            );

            ApiResponse<NgoProfileResponse> body = response.getBody();
            if (body == null || !body.success() || body.data() == null) {
